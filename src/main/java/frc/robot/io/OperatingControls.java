...
public class OperatingControls {
    ...
    public OperatingControls(CommandXboxController operatingController) {
        operatingController.leftTrigger(0.1).onTrue(
                SubsystemManager.getComplexIntakeCommand().andThen(new HomeAmp())); // Uses intake then homes amp
        ...
        operatingController.povUp().onTrue(new HardCodeShot(ShooterConstants.FIRST_AUTON_SHOT_CONFIGURATION)); // Preset angle and shooting speed for shooting

        operatingController.y().whileTrue(new ScoreAmp()); // Scores into the amp

        operatingController.x().whileTrue(new RunCommand(() -> {
                SwerveDrivetrain.getInstance().setDriveConfig(DrivetrainConstants.SHUTTLE_DRIVE_CONFIG); // Prepares Drivetrain for shuttling
                Shooter.getInstance().setShootingConfigSetpoints(ShooterConstants.SHUTTLE_CONFIGURATION); // Prepates shooter for shuttling
                LEDManager.getInstance().setState(LEDState.kTargeting); 
        }).finallyDo(() -> {
                SwerveDrivetrain.getInstance().setDriveConfig(DrivetrainConstants.DEFAULT_DRIVE_CONFIG); // Resets Drivetrain for normal driving
                Shooter.getInstance().setShootingConfigSetpoints(ShooterConstants.DEFAULT_CONFIGURATION); // Resets Shooter for normal shooting
        ...
        operatingController.leftStick().whileTrue(new RunCommand(() -> {
                ...
                SteelTalonsLocalization.getInstance().resetReturnPoseRot(); // Resets the localization to the location before the amp was used
